...
export function modifySessionCookie() {
	if (browser) {
		...
		// The server should only disable `httpOnly` on the session cookie; domain=
		...
		if (session && sig) {
			if (session[0] === '__Secure-cristata-session') {
				document.cookie = `${session[0]}=${session[1]}; SameSite=None; Secure; Expires=Tue, 19 Jan 2038 04:14:07 GMT; domain=.cristata.app`;
				document.cookie = `${sig[0]}=${sig[1]}; SameSite=None; Secure; Expires=Tue, 19 Jan 2038 04:14:07 GMT; domain=.cristata.app`;
			} else {
